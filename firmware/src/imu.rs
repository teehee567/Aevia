//! Volatile SCH16T-K01 acquisition for the trajectory proof of concept.
//!
...
// average. This POC does not perform high-rate coning/sculling compensation.
...
#[allow(dead_code)] // Payload fields are rendered by Debug in the hardware console.
#[derive(Clone, Copy, Debug)]
pub enum ImuError {
    Spi,
    Crc(u64),
...
impl<'d> Imu<'d> {
    ...
    pub async fn next_sample(&mut self) -> Result<ImuSample, ImuError> {
        ...
        loop {
            ...
            let (rates, accelerations, counter) = match self.read_native() {
                Ok(sample) => sample,
                Err(error) => {
                    self.last_counter = None;
                    self.last_time_us = None;
    ...
    fn read_native(&mut self) -> Result<([i32; 3], [i32; 3], [u8; 6]), ImuError> {
...
fn request_frame(address: u8, write: Option<u16>) -> u64 {
...
fn counters_are_consecutive(previous: [u8; 6], current: [u8; 6]) -> bool {
    previous
        .into_iter()
        .zip(current)
...
fn signed_sensor(frame: u64) -> i32 {
    // Signed 20-bit payload occupies bits 27:8.
    (((frame >> 8) as u32 & 0x000f_ffff) << 12) as i32 >> 12
}

fn validate_response(frame: u64, expected: u8, strict_status: bool) -> Result<(), ImuError> {
