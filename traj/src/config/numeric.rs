...
/// Stable identity of the bounded native-`f64` Taylor enclosure currently
/// implemented in the private trajectory root and derivative modules.
///
/// This is deliberately *not* the plan's `EnclosureV1`: it does not claim a
/// `SoftF32` implementation or formal interval inclusion.  Keeping a distinct
/// identity prevents a qualification report for this development backend from
/// being mistaken for the eventual release backend.
...
impl LiveRootEnclosureQualificationV1 {
    pub(super) fn validate_against(
        ...
        if self.backend_id != NATIVE_F64_TAYLOR_ROOT_BACKEND_ID
            || self.backend_revision != NATIVE_F64_TAYLOR_ROOT_BACKEND_REVISION
